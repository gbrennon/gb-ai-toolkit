...
class TestHttpMcpInstaller:
    ...
    @pytest.mark.unit
    def test_install_when_env_in_environ_then_returns_true(
    ...
    @pytest.mark.unit
    def test_install_when_env_in_dotenv_then_returns_true(
    ...
    @pytest.mark.unit
    def test_install_when_env_placeholder_then_returns_true(
        self, http_server, monkeypatch
    ):
        monkeypatch.delenv("GITHUB_TOKEN", raising=False)
        installer = HttpMcpInstaller(dotenv_path=None)
