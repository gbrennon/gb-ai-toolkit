...
description: >-
  ...
  commits by enforcing one commit per changed file. Never skips git hooks unless
...
Never pass `--no-verify`, `-n`, or any flag that bypasses git hooks when
committing. Hooks enforce quality — skipping them hides failures.

Only use `--no-verify` if the user explicitly states "skip hooks" or "bypass
hooks". Never infer implicit consent from user impatience or prior hook
failures. If hooks fail, report the failure and fix it — do not bypass.
...
Do not pass `--no-verify` or `-n` unless the user explicitly requested hook
bypass.
